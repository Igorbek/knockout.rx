...
(function (factory) {
    ...
    function ko2rx(event) {
        ...
        return rx.Observable.create(function (observer) { return _this.subscribe(observer.onNext, observer, event); });
    }
    function ko2rxReply(event) {
        var _this = this;
        return rx.Observable.create(function (observer) {
            observer.onNext(_this.peek());
            return _this.subscribe(observer.onNext, observer, event); // KnockoutSubscription implements Rx._IDisposable
        });
    }
    function ko2rxSubject() {
        var observable = this;
        return rx.Subject.create(rx.Observer.create(function (value) { return observable(value); }), observable.toObservable());
    }
    function rx2koSubscribable() {
        ...
        subscribable.subscribe = function (callback, target, event) {
            var observer = (!event || event == "change" || event == "onNext") ? Rx.Observer.create(callback)
                : (event == "onError" || event == "error") ? Rx.Observer.create(noop, callback)
                    : (event == "onCompleted" || event == "complete") ? Rx.Observer.create(noop, noop, callback)
                        : null;
            if (!observer)
        ...
        subscribable.getSubscriptionsCount = function () { return subscriptionCount; };
        return subscribable;
    }
    ...
    function rxSubject2koObservable(initialValue) {
        ...
        observable.subscribe(function (value) { return !changingBySubject && subject.onNext(value); });
